...
export default async function deleteProductPicture(productID: string, pictureToDeleteID: string | number) {
    ...
    try {
        const res = await fetch(`/api/delete-product-picture/${productID}/${pictureToDeleteID}`, {
            method: 'delete',
            credentials: 'include'
        });
